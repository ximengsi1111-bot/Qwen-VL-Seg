...
#SBATCH --job-name=qvlseg-train248k-remain
#SBATCH --output=/mingli01/project/xiyongkai/qwen3vl-seg/logs/train248k-remain-%j.out
#SBATCH --error=/mingli01/project/xiyongkai/qwen3vl-seg/logs/train248k-remain-%j.err
set -e
...
export PYTHONPATH=.
mkdir -p logs
OUT=/mingli01/data/xyk/runs/eval/train248k-remain
mkdir -p "$OUT"
...
torchrun --nproc_per_node=4 --master_port=$PORT \
  ...
  --data-root "$DATA_ROOT" \
  --manifest "$DATA_ROOT/datasets/small/samples.remaining.jsonl" \
  --splits train \
  --max-pixels 589824 \
  --max-samples 0 \
  --output-dir "$OUT" \
