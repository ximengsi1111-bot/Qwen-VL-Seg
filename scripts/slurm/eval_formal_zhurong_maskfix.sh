...
#SBATCH --job-name=qvlseg-eval-formal-maskfix
...
python3 -m qwen3vl_seg.eval.runner \
  --checkpoint "$DATA_ROOT/checkpoints/small/stage2-100k-maskfix" \
  ...
  --max-pixels 262144 \
  --max-samples 0 \
  --output-dir "$DATA_ROOT/runs/eval/stage2-100k-maskfix-formal"
