...
def main() -> int:
    ...
    def _batch_encode(msgs, add_gen):
        ...
        for m in msgs:
            e = wrapper.processor.apply_chat_template(
                m, tokenize=True, add_generation_prompt=add_gen,
        ...
        def _pad(key_, val):
            return torch.stack([
                F.pad(e[key_].detach().squeeze(0), (max_len - e[key_].shape[1], 0), value=val)
                for e in encs
            ], dim=0)
        return {
    ...
    def process_group(indices, bucket):
        ...
        for s0 in range(0, len(indices), bsz):
            ids = indices[s0:s0 + bsz]
            chunk_s = [dataset.samples[i] for i in ids]
            chunk_item = [dataset[i] for i in ids]
            chunk_img = []
            msgs = []
            for s in chunk_s:
